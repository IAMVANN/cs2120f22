...
prove (X ∨ Y); for then you will need only to apply the __Or_Elim____
...
you answer.
No, it is not the case that (X Or Y is true). When Z is true, that means (X → Z)
and (Y → Z) will always be true regardless of what X or Y is respectively. 
Hence it doesn't matter if (x or y is false) as (X → Z)
and (Y → Z)  would be true if x or y are both false.


Suppose it's raining OR the sprinkler is running, and that in
either case the grass is wet. Is the grass wet? How would you
prove it?
We would first assume that its either raining or that the sprinkler is running.
Since we also know that both cases leads to the grass being wet, we can just use or_elim to prove
to prove that the grass is wet. 
-/
